/*
 * font.c – embedded 8×8 monospace bitmap font (public-domain VGA-style).
 *
 * Each glyph is 8 bytes: one byte per row, MSB = leftmost pixel.
 * Index 0 = ASCII 0x20 (space), index 94 = ASCII 0x7E (~).
 */

#include "font.h"
#include "fb.h"

/* 95 printable ASCII glyphs: 0x20 (space) … 0x7E (~) */
...
/* Fallback glyph used for characters outside printable ASCII. */
static const uint8_t glyph_unknown[8] = {
    0x00,0x7E,0x5A,0x7E,0x5A,0x7E,0x00,0x00
};
...
void draw_char(uint32_t *fb, uint32_t pitch_px,
    ...
    for (int row = 0; row < FONT_HEIGHT; row++) {
        ...
        for (int col = 0; col < FONT_WIDTH; col++) {
            if (bits & (0x80 >> col))
                fb[(y + row) * pitch_px + (x + col)] = colour;
        }
    }
}
...
void draw_string(uint32_t *fb, uint32_t pitch_px,
    ...
    int cx = x;
    while (*s) {
        draw_char(fb, pitch_px, cx, y, (unsigned char)*s, colour);
        cx += FONT_WIDTH;
        s++;
    }
}
