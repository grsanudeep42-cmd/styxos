...
def main():
    ...
    # 3. Wait for boot menu or derived key hash
    print("Waiting for key authorization / boot menu...")
    while True:
        if os.path.exists(LOG_FILE):
            with open(LOG_FILE, "r") as f:
                content = f.read()
                if "PRESS 't' FOR M9" in content or "[M9/M10]" in content:
                    break
        time.sleep(0.2)

    print("Boot menu active. Sending 's' for M10 Snapshot Tests...")
    time.sleep(0.5)
    send_cmd("sendkey s")

    # 4. Wait for test suite completion
    print("Waiting for M10 tests completion...")
    start_t = time.time()
    success = False
    failed = False
    ...
    while time.time() - start_t < 90:
        if os.path.exists(LOG_FILE):
            with open(LOG_FILE, "r") as f:
                content = f.read()
                if "M10 INTEGRATION TESTS COMPLETED SUCCESSFULLY" in content:
                    success = True
                    break
    ...
    print("\n=========================================")
    print(" M10 SNAPSHOT INTEGRATION TEST LOG")
    print("=========================================")
    with open(LOG_FILE, "r") as f:
        print(f.read())
    print("=========================================")
    ...
    if success:
        print("\nRESULT: ALL M10 TESTS PASSED ✓")
        sys.exit(0)
    elif failed:
        print("\nRESULT: M10 TESTS FAILED ✗")
        sys.exit(1)
    else:
