...
def main():
    ...
    print("Boot menu active. Sending 'h' for M12 Interactive Security Shell...")
    send_cmd("sendkey h")
    time.sleep(0.1)
    send_cmd("sendkey h")

    # 4. Wait for test suite completion
    print("Waiting for M12 shell test completion...")
    start_t = time.time()
    success = False
    failed = False
    ...
    while time.time() - start_t < 45:
        if os.path.exists(LOG_FILE):
            with open(LOG_FILE, "r") as f:
                content = f.read()
                if "M12 INTEGRATION TESTS COMPLETED SUCCESSFULLY" in content:
                    success = True
                    break
    ...
    print("\n=========================================")
    print(" M12 INTERACTIVE SECURITY SHELL LOG")
    print("=========================================")
    with open(LOG_FILE, "r") as f:
        print(f.read())
    print("=========================================")
    ...
    if success:
        print("\nRESULT: ALL M12 TESTS PASSED ✓")
        sys.exit(0)
    elif failed:
        print("\nRESULT: M12 TESTS FAILED ✗")
        sys.exit(1)
    else:
