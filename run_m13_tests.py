...
def main():
    ...
    print("Boot menu active. Sending 'v' for M13 TPM 2.0 Attestation Tests...")
    send_cmd("sendkey v")
    time.sleep(0.1)
    send_cmd("sendkey v")

    # 4. Wait for test suite completion
    print("Waiting for M13 TPM test completion...")
    start_t = time.time()
    success = False
    failed = False
    ...
    while time.time() - start_t < 45:
        if os.path.exists(LOG_FILE):
            with open(LOG_FILE, "r") as f:
                content = f.read()
                if "M13 INTEGRATION TESTS COMPLETED SUCCESSFULLY" in content:
                    success = True
                    break
    ...
    print("\n=========================================")
    print(" M13 TPM 2.0 ATTESTATION TEST LOG")
    print("=========================================")
    with open(LOG_FILE, "r") as f:
        print(f.read())
    print("=========================================")
    ...
    if success:
        print("\nRESULT: ALL M13 TESTS PASSED ✓")
        sys.exit(0)
    elif failed:
        print("\nRESULT: M13 TESTS FAILED ✗")
        sys.exit(1)
    else:
